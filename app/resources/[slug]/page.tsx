...
export default async function ResourcePage({ params }: ResourcePageProps) {
  ...
  const currentIndex = resourcePosts.findIndex((p) => p.slug === slug);
  const prevPost = currentIndex > 0 ? resourcePosts[currentIndex - 1] : null;
  const nextPost = currentIndex < resourcePosts.length - 1 ? resourcePosts[currentIndex + 1] : null;
